...
export default function MCPServerDetail({ process, onKill }: MCPServerDetailProps) {
  ...
  const getRAMStatus = (ramMB: number): { text: string; color: string } => {
    if (ramMB >= 500) return { text: "High", color: "🔴" };
    if (ramMB >= 200) return { text: "Medium", color: "🟠" };
    if (ramMB >= 100) return { text: "Moderate", color: "🟡" };
...
| **CPU** | ${process.cpuPercentage.toFixed(2)}% | ${process.cpuPercentage > 50 ? "🔴 High" : process.cpuPercentage > 20 ? "🟠 Medium" : "🟢 Low"} |
        ...
        <Detail.Metadata>
          ...
          <Detail.Metadata.TagList title="Source">
            <Detail.Metadata.TagList.Item
              ...
              color={
                process.source === "claude-desktop"
                  ? Color.Orange
                  : process.source === "vscode"
                    ? Color.Blue
                    : process.source === "cursor"
                      ? Color.Purple
                      : process.source === "claude-code"
                        ? Color.Green
                        : Color.SecondaryText
              }
            />
